...
def mock_graph(mocker: 'MockerFixture') -> MagicMock:
    ...
    return mocker.patch("src.photo_culling_agent.gradio_interface.gradio_interface.PhotoCullingGraph")
...
def interface_instance(mocker: 'MockerFixture', tmp_path: str) -> PhotoCullingInterface:
    ...
    mocker.patch("src.photo_culling_agent.gradio_interface.gradio_interface.PhotoCullingGraph", return_value=mock_pipeline_instance)
    
    decision_weights = {"composition": 1.0, "exposure": 1.0}
    interface = PhotoCullingInterface(output_dir=str(tmp_path / "output"), decision_weights=decision_weights)
...
class TestPhotoCullingInterface:
    ...
    def test_init(self, mocker: 'MockerFixture', tmp_path: str) -> None:
        ...
        mock_tempfile_mkdtemp = mocker.patch("tempfile.mkdtemp", return_value=str(tmp_path / "test_temp_dir"))
        mock_pipeline_init = mocker.patch("src.photo_culling_agent.gradio_interface.gradio_interface.PhotoCullingGraph")
    ...
    def test_del_cleanup(self, mocker: 'MockerFixture', tmp_path: str) -> None:
        ...
        mocker.patch("src.photo_culling_agent.gradio_interface.gradio_interface.PhotoCullingGraph") # Mock graph init
    ...
    def test_del_cleanup_exception(self, mocker: 'MockerFixture', tmp_path: str) -> None:
        ...
        mock_os_path_exists.assert_called_with(temp_dir_path)
        mock_shutil_rmtree.assert_called_once_with(temp_dir_path)
        mock_print.assert_called_once_with(f"Error cleaning up temp directory: Test error")
